...
def handle_borders(facel, facer, sides_detected):
    ...
    if len(sides_detected[facel]['positions']) > 0:
        facel_bnd = sides_detected[facel]['bounding_boxes'][0]
        if 480 - facel_bnd[2] <= 1:
            facel_pos = sides_detected[facel]['positions'][0]
    if len(facel_pos) > 0 and len(facer_pos) > 0:
        ...
        sides_detected[facel]['bounding_boxes'].pop(0)
        sides_detected[facel]['positions'].pop(0)
...
def assign_pose2person(people_detected, j, person, image, pos, model1, galleries, sorted_positions, flag_concat):
    ...
    while j < len(sorted_people):
        ...
        if 0 <= person[0] < 240:
            if 0 <= person[2] < 240:
                ...
                for i, back in enumerate(sides_detected['back']):
                    if 0 <= back['bounding_boxes'][0] < 240:
                        if 240 <= back['bounding_boxes'][2] < 480:
                ...
                if flag_concat:
                    ...
                    galleries, sorted_positions, people_detected = assign_pose2person(
                        people_detected, j, person, img, pos, model1, galleries, sorted_positions, flag_concat
                    )
                    j += 1
                else:
                    galleries, sorted_positions, people_detected = assign_pose2person(
